...
export function overlaps(tree1, tree2) {
  return cowalk(tree1, tree2, (node1, node2, next) => {
...
export function getPayloads(object, changes, listeners) {
  if (typeof listeners === 'undefined' || typeof changes === 'undefined') return;
  if (listeners[KEYS] && listeners[KEYS].length) {
    ...
    if (added.length || removed.length) {
      ...
      } catch (e) { if (process.env.NODE_ENV !== 'production') throw e; }
    }
  }
  ...
  for (const prop in listeners) {
    if (
      hasProp(changes, prop) ||
      changes === null && hasProp(object, prop)
    ) invokeListeners(
      object && object[prop], changes && changes[prop], listeners[prop]
    );
  }
}
...
function diff(state, change, replace) {
  ...
  for (const prop in change) {
    const difference =
      isEqual(state[prop], change[prop]) ? undefined :
      !hasProp(state, prop) || isLeaf(change, prop) || isLeaf(state, prop) ? change[prop] :
  ...
  if (replace) for (const prop in state) {
