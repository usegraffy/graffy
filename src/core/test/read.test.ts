...
test('simple', async () => {
  ...
  assert.deepStrictEqual(await g.read('foo', { bar: { baz: 1 } }), {
    bar: { baz: 42 },
  });
});
...
test('overlap', async () => {
  ...
  assert.deepStrictEqual(
    await g.read({ foo: { bar: { x: 1 }, baz: { x: 1 } } }),
...
test('remove_null', async () => {
  ...
  assert.deepStrictEqual(await g.read({ foo: { bar: 1, baz: 1 } }), {
    foo: { bar: 45, baz: null },
  });
});
...
test('getKnown', async () => {
  ...
  assert.deepStrictEqual(await g.read({ foo: { bar: 1 } }), {
    foo: { bar: 42 },
  });
});
...
describe('link', () => {
  ...
  test('friendly', async () => {
    assert.deepStrictEqual(await g.read({ foo: { x: { baz: 1 } } }), {
      foo: { x: ref(['bar'], { baz: 3 }) },
    });
  });
});
...
describe('alias', () => {
  test('simple', async () => {
    g.onRead('foo', () => ({ x: 100 }));
    assert.deepStrictEqual(await g.read({ bar: { $ref: ['foo'], x: 1 } }), {
      bar: ref(['foo'], { x: 100 }),
    });
  });
...
describe('middleware_coding', () => {
  test('no_path', async () => {
    ...
    g.onRead(async (query, options, next) => {
      ...
      assert.deepStrictEqual(result, { participant: decomposedResult });
      return result;
    });
